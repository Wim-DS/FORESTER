...
s_richness <- s_tidy_complete %>% 
  group_by(Session, Location, Treetype, Species) %>% 
...
s_richness_pool <- s_tidy_complete %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
  filter(Count != 0) %>% 
...
s_richness_pool <- as.data.frame(s_richness_pool) %>% 
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
            Treetype == "P" & Location %in% c("P1", "P2", "P3", "P5", "P6", "P7", "P8")))

s_richness_pool$Treetype <- factor(s_richness_pool$Treetype, 
                            levels = c("B", "O", "P"))

s_richness_pool_spring <- s_tidy_complete %>% 
  filter(Session < 5) %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
...
s_richness_pool_spring <- as.data.frame(s_richness_pool_spring) %>% 
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
...
s_richness_pool_spring$Treetype <- factor(s_richness_pool_spring$Treetype, 
                            levels = c("B", "O", "P"))

s_richness_pool_summer <- s_tidy_complete %>% 
  filter(Session > 4) %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
...
s_richness_pool_summer <- as.data.frame(s_richness_pool_summer) %>% 
  add_row(Location = "O3", Treetype = "O") %>%
  add_row(Location = "O5", Treetype = "O") %>%
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
...
s_richness_pool_summer$Treetype <- factor(s_richness_pool_summer$Treetype, 
                            levels = c("B", "O", "P"))
...
b_richness_pool <- b_tidy_complete %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
  filter(Count != 0) %>% 
...
b_richness_pool <- as.data.frame(b_richness_pool) %>% 
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
...
b_richness_pool$Treetype <- factor(b_richness_pool$Treetype, 
                            levels = c("B", "O", "P"))

b_richness_pool_spring <- b_tidy_complete %>% 
  filter(Session < 5) %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
...
b_richness_pool_spring <- as.data.frame(b_richness_pool_spring) %>% 
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
...
b_richness_pool_spring$Treetype <- factor(b_richness_pool_spring$Treetype, 
                            levels = c("B", "O", "P"))

b_richness_pool_summer <- b_tidy_complete %>% 
  filter(Session > 4) %>% 
  group_by(Location, Treetype, Species) %>% 
  summarise(Count = sum(Count)) %>% 
  ungroup() %>% 
...
b_richness_pool_summer <- as.data.frame(b_richness_pool_summer) %>% 
  add_row(Location = "B3", Treetype = "B") %>% 
  add_row(Location = "O3", Treetype = "O") %>% 
  add_row(Location = "O5", Treetype = "O") %>% 
  add_row(Location = "P1", Treetype = "P") %>% 
  add_row(Location = "P6", Treetype = "P") %>%
  add_row(Location = "P7", Treetype = "P") %>%
  add_row(Location = "P8", Treetype = "P") %>%
  complete(Location, Treetype) %>%
  mutate(across(everything(), replace_na, 0)) %>% 
  subset((Treetype == "B" & Location %in% c("B2", "B3", "B4", "B5", "B6", "B8", "B9")|
            Treetype == "O" & Location %in% c("O3", "O4", "O5", "O6", "O7")|
...
b_richness_pool_summer$Treetype <- factor(b_richness_pool_summer$Treetype, 
                            levels = c("B", "O", "P"))
...
```{r rarefaction iNEXT, echo=FALSE, warning=FALSE, message=FALSE}
...
out <- iNEXT(bird, q = 0, datatype = 'abundance') #q=0, want we willen species richness, geen diversity maat
out
 
out1 <- iNEXT(bird, q=0, datatype="abundance")
ggiNEXT(x=out1, type=1)
 
chao <- ChaoRichness(bird, datatype = "abundance", conf = 0.95) |> #chao estimates berekenen, hieraan kan dan andere variabelen zoals SCI toegevoegd worden met left_join
  rownames_to_column()
 
ggplot(chao, aes(rowname, Estimator)) + #chao estimates visualiseren
  geom_point() +
...
# BEES ---------------------------------------------------------------------------
b_ab_rarefaction <- bees %>% pivot_wider(names_from = "Location", values_from = "Count") %>% 
  mutate(across(everything(), replace_na, 0)) %>% 
  group_by(Species) %>% 
  summarise_if(is.numeric, sum, na.rm = TRUE) %>% 
  dplyr::select(-Trap, -Session) %>% 
  column_to_rownames(., "Species")

b_out <- iNEXT(b_ab_rarefaction, q = 0, datatype = 'abundance')
b_out

b_chao <- ChaoRichness(b_ab_rarefaction, datatype = "abundance", conf = 0.95) %>% rownames_to_column(var = "Location") %>% 
  rename(b_estimator = "Estimator", b_observed = "Observed", b_est_se = "Est_s.e.", b_est_lower = "95% Lower", b_est_upper = "95% Upper")

ggplot(b_chao, aes(Location, b_estimator)) + #chao estimates visualiseren
  geom_point() +
  geom_errorbar(aes(ymin = b_est_lower, ymax = b_est_upper)) +
  theme_classic()

# SYRPHIDS -----------------------------------------------------------------------

s_ab_rarefaction <- syrphids %>% pivot_wider(names_from = "Location", values_from = "Count") %>% 
  mutate(across(everything(), replace_na, 0)) %>% 
  group_by(Species) %>% 
  summarise_if(is.numeric, sum, na.rm = TRUE) %>% 
  dplyr::select(-Trap, -Session) %>% 
...
s_out <- iNEXT(s_ab_rarefaction, q = 0, datatype = 'abundance')
s_out

s_chao <- ChaoRichness(s_ab_rarefaction, datatype = "abundance", conf = 0.95) %>% rownames_to_column(var = "Location")%>% 
  rename(s_estimator = "Estimator", s_observed = "Observed", s_est_se = "Est_s.e.", s_est_lower = "95% Lower", s_est_upper = "95% Upper")

ggplot(s_chao, aes(Location, s_estimator)) + #chao estimates visualiseren
  geom_point() +
  geom_errorbar(aes(ymin = s_est_lower, ymax = s_est_upper)) +
  theme_classic()
...
bees_variables_pool <- b_abundance %>% 
  group_by(Location, Treetype) %>% 
  summarise(b_AB = sum(Count)) %>% 
  left_join(., b_richness_pool, by =c("Location", "Treetype")) %>% 
  rename(b_SR = "SR")

bees_variables_pool_spring <- b_abundance %>% 
  mutate(Session = as.numeric(Session)) %>% 
  filter(Session < 5) %>% 
  group_by(Location, Treetype) %>% 
  summarise(b_AB = sum(Count)) %>% 
  left_join(., b_richness_pool_spring, by =c("Location", "Treetype")) %>% 
  rename(b_SR = "SR")

bees_variables_pool_summer <- b_abundance %>%
  mutate(Session = as.numeric(Session)) %>%
  filter(Session > 4) %>% 
  group_by(Location, Treetype) %>% 
  summarise(b_AB = sum(Count)) %>% 
  left_join(., b_richness_pool_summer, by =c("Location", "Treetype")) %>% 
  rename(b_SR = "SR")
...
syrphids_variables_pool <- s_abundance %>% 
  group_by(Location, Treetype) %>% 
  summarise(s_AB = sum(Count)) %>% 
  left_join(., s_richness_pool, by =c("Location", "Treetype")) %>% 
  rename(s_SR = "SR")

syrphids_variables_pool_spring <- s_abundance %>% 
  mutate(Session = as.numeric(Session)) %>%
  filter(Session < 5) %>% 
  group_by(Location, Treetype) %>% 
  summarise(s_AB = sum(Count)) %>% 
  left_join(., s_richness_pool_spring, by =c("Location", "Treetype")) %>% 
  rename(s_SR = "SR")

syrphids_variables_pool_summer <- s_abundance %>% 
  mutate(Session = as.numeric(Session)) %>%
  filter(Session > 4) %>% 
  group_by(Location, Treetype) %>% 
  summarise(s_AB = sum(Count)) %>% 
  left_join(., s_richness_pool_summer, by =c("Location", "Treetype")) %>% 
  rename(s_SR = "SR")
...
flowerdata_pool_manual <- read.csv("Flowerdata_pool.csv")

flowerdata_pool <- flowerdata %>% 
  group_by(Location, Treetype) %>% 
  summarise(flower_AB = sum(flower_AB)) %>% 
  left_join(., flowerdata_pool_manual %>% dplyr::select(Location, overall), by = "Location") %>% 
  rename(flower_SR = "overall")

flowerdata_pool_spring <- flowerdata %>% 
  filter(Session < 5) %>% 
  group_by(Location, Treetype) %>% 
  summarise(flower_AB = sum(flower_AB)) %>% 
  left_join(., flowerdata_pool_manual %>% dplyr::select(Location, spring), by = "Location") %>% 
  rename(flower_SR = "spring")

flowerdata_pool_summer <- flowerdata %>% 
  filter(Session > 4) %>% 
  group_by(Location, Treetype) %>% 
  summarise(flower_AB = sum(flower_AB)) %>% 
  left_join(., flowerdata_pool_manual %>% dplyr::select(Location, summer), by = "Location") %>% 
  rename(flower_SR = "summer")
...
Bestuivers_master <- bees_variables %>% 
  ...
  mutate(SCI_discrete = as.factor(ifelse(SCI > 0.52, "high", ifelse(SCI < 0.35, "low", "medium")))) %>% #gelijke klassen tussen min en max (tss 0.18 en 0.69, grootte is 0.17)
  left_join(., Coverdata, by = "Location") %>% 
  left_join(., b_chao , by = "Location") %>% 
  left_join(., s_chao , by = "Location")

# max(Bestuivers_master$SCI) #0.69
# min(Bestuivers_master$SCI) #0.18

#test <- Bestuivers_master %>% group_by(Treetype, SCI_discrete) %>% summarise(Count = n())

Bestuivers_master_spring <- Bestuivers_master %>% filter(Session < 5)
Bestuivers_master_summer <- Bestuivers_master %>% filter(Session > 4)

SCI_discrete <- Bestuivers_master %>% group_by(Location, SCI_discrete) %>% summarise(test = mean(SCI)) %>% dplyr::select(-test)

Bestuivers_master_pool <- bees_variables_pool %>% 
  left_join(., syrphids_variables_pool, by = c("Location", "Treetype")) %>% 
  mutate(best_AB = b_AB + s_AB, best_SR = b_SR + s_SR) %>%
  mutate(Treetype = as.character(Treetype)) %>% 
  left_join(., SCI_table, by = c("Location", "Treetype")) %>% 
  mutate(SCI_discrete = as.factor(ifelse(SCI > 0.52, "high", ifelse(SCI < 0.35, "low", "medium")))) %>%
  left_join(., Coverdata, by = "Location") %>% 
  left_join(., flowerdata_pool, by = c("Location","Treetype"))%>% 
  left_join(., b_chao , by = "Location") %>% 
  left_join(., s_chao , by = "Location")

Bestuivers_master_pool_spring <- bees_variables_pool_spring %>% 
  left_join(., syrphids_variables_pool_spring, by = c("Location", "Treetype")) %>% 
  mutate(best_AB = b_AB + s_AB, best_SR = b_SR + s_SR) %>%
  mutate(Treetype = as.character(Treetype)) %>% 
  left_join(., SCI_table, by = c("Location", "Treetype")) %>% 
  mutate(SCI_discrete = as.factor(ifelse(SCI > 0.52, "high", ifelse(SCI < 0.35, "low", "medium")))) %>%
  left_join(., flowerdata_pool_spring, by = c("Location","Treetype")) %>% 
  left_join(., Coverdata, by = "Location")%>% 
  left_join(., b_chao , by = "Location") %>% 
  left_join(., s_chao , by = "Location")

Bestuivers_master_pool_summer <- bees_variables_pool_summer %>% 
  left_join(., syrphids_variables_pool_summer, by = c("Location", "Treetype")) %>% 
  mutate(best_AB = b_AB + s_AB, best_SR = b_SR + s_SR) %>%
  mutate(Treetype = as.character(Treetype)) %>% 
  left_join(., SCI_table, by = c("Location", "Treetype")) %>% 
  mutate(SCI_discrete = as.factor(ifelse(SCI > 0.52, "high", ifelse(SCI < 0.35, "low", "medium")))) %>%
  left_join(., flowerdata_pool_summer, by = c("Location","Treetype")) %>% 
  left_join(., Coverdata, by = "Location")%>% 
  left_join(., b_chao , by = "Location") %>% 
  left_join(., s_chao , by = "Location")
...
ggplot(Bestuivers_master_pool, aes(x = SCI, y = b_estimator, fill = Treetype, colour = Treetype)) + geom_point() + geom_smooth(method = "lm")
...
#MODEL 1 UNPOOLED ___________________________________________________________________________________________________________________
##BEES-----------------------------------------------------------------------------

b_model1 <- glmer(data = Bestuivers_master, b_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model1)

ggplot(Bestuivers_master, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

b_model1_spring <- glmer(data = Bestuivers_master_spring, b_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model1_spring)

ggplot(Bestuivers_master_spring, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

b_model1_summer <- glmer(data = Bestuivers_master_summer, b_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model1_summer)

ggplot(Bestuivers_master_summer, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

plot_models(b_model1, b_model1_spring, b_model1_summer,
            m.labels = c("All season", "Spring", "Summer"),
            colors = c("#56B4E9","#009E73","#D55E00"),
            legend.title = "Bee abundance",
            vline.color = "black",
...
s_model1 <- glmer(data = Bestuivers_master, s_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model1)

ggplot(Bestuivers_master, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

s_model1_spring <- glmer(data = Bestuivers_master_spring, s_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model1_spring)

ggplot(Bestuivers_master_spring, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

s_model1_summer <- glmer(data = Bestuivers_master_summer, s_AB ~ SCI + Session + SCI:Session + Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model1_summer)

ggplot(Bestuivers_master_summer, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point(aes(col = Treetype)) +
  facet_grid(~ Session)

plot_models(s_model1, s_model1_spring, s_model1_summer,
            m.labels = c("All season", "Spring", "Summer"),
            colors = c("#56B4E9","#009E73","#D55E00"),
            legend.title = "Syrphid abundance",
            vline.color = "black",
            axis.lim = c(0.01,100),
            show.values = T,
            show.p =T,
...
#MODEL 2 POOLED _____________________________________________________________________________________________________________________
##BEES-----------------------------------------------------------------------------

b_model2 <- glmer(data = Bestuivers_master_pool, b_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model2)

ggplot(Bestuivers_master_pool, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

b_model2_spring <- glmer(data = Bestuivers_master_pool_spring, b_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model2_spring)

ggplot(Bestuivers_master_pool_spring, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

b_model2_summer <- glmer(data = Bestuivers_master_pool_summer, b_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(b_model2_summer)

ggplot(Bestuivers_master_pool_summer, aes(SCI, b_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

plot_models(b_model2, b_model2_spring, b_model2_summer,
            m.labels = c("All season", "Spring", "Summer"),
            colors = c("#56B4E9","#009E73","#D55E00"),
            legend.title = "Bee abundance pooled",
            vline.color = "black",
...
s_model2 <- glmer(data = Bestuivers_master_pool, s_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model2)

ggplot(Bestuivers_master_pool, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

s_model2_spring <- glmer(data = Bestuivers_master_pool_spring, s_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model2_spring)

ggplot(Bestuivers_master_pool_spring, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

s_model2_summer <- glmer(data = Bestuivers_master_pool_summer, s_AB ~ SCI + Treetype + SCI:Treetype + flower_AB + flower_SR + (1|Location), family = poisson(link = "log")) 
summary(s_model2_summer)

ggplot(Bestuivers_master_pool_summer, aes(SCI, s_AB,  col = Treetype)) +
  stat_smooth(method="glm", method.args = list(family = "poisson")) +
  geom_point()+
  facet_grid(~ Treetype)

plot_models(s_model2, s_model2_spring, s_model2_summer,
            m.labels = c("All season", "Spring", "Summer"),
            colors = c("#56B4E9","#009E73","#D55E00"),
            legend.title = "Syrphid abundance pooled",
            vline.color = "black",
            #axis.lim = c(0.01,1000),
            show.values = T,
            show.p =T,
...
colnames(bees_data_scores)[1] <- "fullname"
bees_data_scores <- bees_data_scores %>% 
  mutate(Treetype = str_extract(fullname,"[BOP]")) %>% 
  mutate(Location = str_extract(fullname, "\\d")) %>% 
  mutate(Location = str_c(Treetype, "" , Location))
bees_data_scores$Treetype <- factor(bees_data_scores$Treetype,
                              levels = c("B", "O", "P"))

bees_data_scores_SCI <- bees_data_scores %>% 
  left_join(., SCI_discrete, by = "Location")
...
bees_low <- bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == "low", ][chull(bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == 
    "low", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI

bees_medium <- bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == "medium", ][chull(bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == 
    "medium", c("NMDS1", "NMDS2")]), ]  # hull values for medium SCI

bees_high <- bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == "high", ][chull(bees_data_scores_SCI[bees_data_scores_SCI$SCI_discrete == 
    "high", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI

bees_SCI_hull_data <- rbind(bees_low, bees_medium, bees_high)  
...
colnames(bees_MDS_xy)[1] <- "fullname"
...
```{r cleanup NMDS, echo=FALSE, warning=FALSE, message=FALSE}
...
species_bees_nmds <- species_bees_nmds %>% 
  left_join(., SCI_discrete, by = "Location") %>% 
  relocate(SCI_discrete, .after = Treetype)

species_bees_nmds <- species_bees_nmds[rowSums(species_bees_nmds[, -c(0:3)])!= 0, ]

rownames(species_bees_nmds) <- paste(species_bees_nmds$Location, species_bees_nmds$Treetype, species_bees_nmds$SCI_discrete)

species_bees_nmds_clean <- species_bees_nmds[, -c(0:3)] 
species_bees_nmds_clean <- as.matrix(species_bees_nmds_clean)
...
bees_data_scores <- bees_data_scores %>% 
  ...
  mutate(Location = str_extract(fullname, "\\d")) %>% 
  mutate(Location = str_c(Treetype, "" , Location)) %>% 
  mutate(SCI = str_extract(fullname, "[:lower:]"))
bees_data_scores$Treetype <- factor(bees_data_scores$Treetype,
                              levels = c("B", "O", "P"))
bees_data_scores$SCI <- factor(bees_data_scores$SCI,
                              levels = c("l", "m", "h"))
...
bees_low <- bees_data_scores[bees_data_scores$SCI == "l", ][chull(bees_data_scores[bees_data_scores$SCI == 
    "l", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI

bees_medium <- bees_data_scores[bees_data_scores$SCI == "m", ][chull(bees_data_scores[bees_data_scores$SCI == 
    "m", c("NMDS1", "NMDS2")]), ]  # hull values for medium SCI

bees_high <- bees_data_scores[bees_data_scores$SCI == "h", ][chull(bees_data_scores[bees_data_scores$SCI == 
    "h", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI
...
bees_MDS_xy$SCI <- factor(bees_MDS_xy$SCI, 
                         levels = c("l", "m", "h"))
...
species_syrphids_nmds <- species_syrphids_nmds %>% 
  left_join(., SCI_discrete, by = "Location") %>% 
  relocate(SCI_discrete, .after = Treetype)

species_syrphids_nmds <- species_syrphids_nmds[rowSums(species_syrphids_nmds[, -c(0:3)])!= 0, ]

rownames(species_syrphids_nmds) <- paste(species_syrphids_nmds$Location, species_syrphids_nmds$Treetype, species_syrphids_nmds$SCI_discrete)
...
syrphids_data_scores <- as.data.frame(scores(syrphids_nmds_results)$sites)
syrphids_data_scores <- rownames_to_column(syrphids_data_scores)
colnames(syrphids_data_scores)[1] <- "fullname"
syrphids_data_scores <- syrphids_data_scores %>% 
  mutate(Treetype = str_extract(fullname,"[BOP]")) %>% 
  mutate(Location = str_extract(fullname, "\\d")) %>% 
  mutate(Location = str_c(Treetype, "" , Location)) %>% 
...
syrphids_data_scores$Treetype <- factor(syrphids_data_scores$Treetype,
                              levels = c("B", "O", "P"))
syrphids_data_scores$SCI <- factor(syrphids_data_scores$SCI,
                              levels = c("l", "m", "h"))
...
syrphids_low <- syrphids_data_scores[syrphids_data_scores$SCI == "l", ][chull(syrphids_data_scores[syrphids_data_scores$SCI == 
    "l", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI

syrphids_medium <- syrphids_data_scores[syrphids_data_scores$SCI == "m", ][chull(syrphids_data_scores[syrphids_data_scores$SCI == 
    "m", c("NMDS1", "NMDS2")]), ]  # hull values for medium SCI

syrphids_high <- syrphids_data_scores[syrphids_data_scores$SCI == "h", ][chull(syrphids_data_scores[syrphids_data_scores$SCI == 
    "h", c("NMDS1", "NMDS2")]), ]  # hull values for low SCI

syrphids_SCI_hull_data <- rbind(syrphids_low, syrphids_medium, syrphids_high)  

syrphids_MDS_xy <- data.frame(syrphids_nmds_results$points)
syrphids_MDS_xy <- rownames_to_column(syrphids_MDS_xy)
colnames(syrphids_MDS_xy)[1] <- "fullname"
syrphids_MDS_xy <- syrphids_MDS_xy %>% 
  mutate(Treetype = str_extract(fullname,"[BOP]")) %>% 
  mutate(Location = str_extract(fullname, "\\d")) %>% 
  mutate(Location = str_c(Treetype, "" , Location)) %>% 
...
syrphids_MDS_xy$SCI <- factor(syrphids_MDS_xy$SCI, 
                         levels = c("l", "m", "h"))
